...
| EPSON Endeavor NJ1000 / Debian 12 i386（32-bit / glibc / EHCI） | Stable v0.1.3 source archive | PC/SC IFD無効のnative Release build 92/92 targetsとCTest 5/5が成功し、`px4d` / `px4-ts` / `px4ctl`のELF 32-bit Intel 80386を確認。PX-Q3U4の8 receiver同時60秒gateと30分soakがpass。30分はreceiver 0〜6が各約39億bytes、全TS/queue/USB error 0。receiver 7のみ既知個体burst（TEI 10,988、continuity 614）。direct APDUは30標本×10回成功、status USB/protocol error 0、daemon FD 29固定・RSS安定、最高56℃、process/endpoint残留0。 | PC/SC IFDは未build・未検証。 |
| Latitude 5300 / AnduinOS（localhost usbip / VHCI） | Stable v0.1.3 x86_64 glibc archive（source ref `639e65f`、SHA-256: `5eabfa041c1d61230b31c0ed370d40f842a741d8523756c826b926f58155269b`） | Linux 7.0.0-31-generic、usbip 2.0。PX-Q3U4の2 USB機能をnetwork namespace内loopbackの`usbipd`から別VHCI portへattach。物理export stubとVHCI importが同一libusb contextへ同一serialで二重列挙されるため、VHCI側2 nodeを既存`px4d --fd FD --fd FD`で指定。BS15_0の60秒gateと30分soakは8/8 clean。30分は各20,756,732〜28,864,141 packets、全TS/queue/USB error 0、status 30/30、direct APDU 30/30、kernel error 0。初回BS13_0 30分はr2/r3/r6のcontinuity error各2によりstrict failとして証跡を保持し、判定例外は追加していない。終了後にTCP 3240、namespace、VHCI、processの残留0、物理interfaceのdriverなし復元を確認。 | 同一ホスト内のusbip/VHCI経路。LAN越し・切断注入は未検証。native列挙時の重複はdescriptor破損ではなく、FD指定で回避。 |
| Latitude 5300 / AnduinOS（AppArmor enforce） | Stable v0.1.3 x86_64 glibc archive（`px4d` SHA-256: `0fa31583b8725183017c0f8b0e5eec437700da9a3d9e5e4c803cce17ef61e6c6`、firmware SHA-256: `5213a5a38872661277a2cc1b2dfdfe88faf06f41205f460f3b51857f0568b484`） | Linux 7.0.0-31-generic、AppArmor 5.0.2。一時的な自作profileで`px4d` native USB openのdeny gate、complain 30秒、enforce 60秒、30分を実施。30分はr0〜r6の全TS/queue/USB error 0、r7は既知個体burst基準内。status 157/157、direct APDU 157/157（全応答末尾SW9000）、AppArmor denial・kernel Oops/OOM/USB reset/disconnect 0。daemon FD 29固定、RSSに単調増加なし。profile/process/runtime残留0。 | AppArmor profileは配布物に含まれない。カードはdaemon direct APDUまでを検証し、IFD/`pcscd`経路は未検証。運用上の注意は[AppArmor文書](apparmor.md)を参照。 |
