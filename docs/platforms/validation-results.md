...
| EPSON Endeavor NJ1000 / Debian 12 i386（32-bit / glibc / EHCI） | Stable v0.1.3 source archive | 合格。PC/SC IFD込みnative Release build（98/98 targets）、CTest 7/7、PC/SC smoke、8受信機同時30分soak、PC/SCおよびdirect APDU完走、物理切断・再接続復帰を確認。r7のみ既知個体バースト再現。 | 実機操作はroot/sudoで実施。詳細は [Debian 12 i386での検証結果](debian-i686.md) を参照。 |
...
| Latitude 5300 / AnduinOS（AppArmor enforce） | Stable v0.1.3 x86_64 glibc archive | 合格。USB拒否gate、complain、enforce各試験完走。30分受信でr0〜r6は全TS/queue/USB error 0、r7は既知個体バースト基準内。direct APDU 157/157成功。 | profileは利用者側で用意する。IFD/pcscd経路は検証対象外。詳細は [AppArmorで実行する際の注意](apparmor.md) を参照。 |
